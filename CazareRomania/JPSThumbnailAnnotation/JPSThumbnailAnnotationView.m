...
- (void)setupImageView {
    ...
    // JOHN modif only -> ! Use with care `cos it's a nice piece :D
    UIImage *punct = [ UIImage imageNamed:@"red-pin.png" ];
    UIImageView *imgView = [ [ UIImageView alloc ] initWithImage:punct ];
    imgView.frame = CGRectMake( _imageView.frame.size.width/2+6, _imageView.frame.size.height+25,31,38);
...
- (void)setupDisclosureButton {
  ...
  //j fix ios 7 sdk  UIButtonType buttonType = iOS7 ? UIButtonTypeSystem : UIButtonTypeCustom;
    UIButtonType buttonType = iOS7 ? UIButtonTypeCustom : UIButtonTypeCustom;
